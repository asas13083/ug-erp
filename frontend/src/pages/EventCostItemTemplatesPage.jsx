...
export default function EventCostItemTemplatesPage() {
  const { t } = useLanguage();
  ...
  function load() {
    api.get('/event-cost-item-templates').then(({ data }) => setTemplates(data.data));
  }
  useEffect(load, []);
  ...
  async function handleAdd(e) {
    ...
    setError('');
    try {
      await api.post('/event-cost-item-templates', { name });
      setName('');
      load();
    } catch (err) {
  ...
  async function saveEdit(id) {
    setError('');
    try {
      await api.put(`/event-cost-item-templates/${id}`, { name: editingName });
      setEditingId(null);
      load();
    } catch (err) {
  ...
  async function handleDelete(p) {
    if (!window.confirm(`${t('متأكد إنك عايز تحذف اسم بند')} "${p.name}"؟`)) return;
    try {
      await api.delete(`/event-cost-item-templates/${p.id}`);
      load();
    } catch (err) {
  ...
  return (
    <>
      <PageHeader title={t('أسماء بنود التوتال الشائعة')} subtitle={t('قايمة الأسماء اللي هتظهرلك تختار منها وقت ما تضيف بند توتال جديد في أي كشف حسابات')} />
      <div className="p-7 max-w-2xl">
        <form onSubmit={handleAdd} className="flex gap-2 mb-5">
          <input
            ...
            placeholder={t('اسم بند جديد (مثال: اجمالي الجبس)')}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          {templates.length === 0 && <div className="col-span-2 text-center py-10 text-gray-600 text-sm">{t('لا توجد أسماء بنود بعد')}</div>}
        </div>
      </div>
    </>
  );
