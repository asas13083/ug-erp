...
const getSummary = asyncHandler(async (req, res) => {
  ...
  const [costItems, entries, supplierEntries] = await Promise.all([
    ...
    prisma.eventSupplierEntry.findMany({ where: { eventId }, select: { total: true, paidAmount: true, paymentAllocations: { select: { amount: true } } } }),
  ]);
  ...
  const suppliersPaid = supplierEntries.reduce(
    (s, e) => s + e.paidAmount + e.paymentAllocations.reduce((a, x) => a + x.amount, 0),
    0
  );
...
async function fetchEventsWithTotals({ q, dateFrom, dateTo, skip, take }) {
  ...
  const [allItems, allEntries, allSupplierEntries, overallItems, overallEntries, overallSupplierEntries] = await Promise.all([
    ...
    prisma.eventSupplierEntry.aggregate({ where: { eventId: { in: allIds } }, _sum: { total: true } }),
  ]);
  ...
  const data = events.map((ev) => {
    const suppliersTotal = suppliersByEvent.get(ev.id) || 0;
    return {
      ...ev,
      suppliersTotal,
